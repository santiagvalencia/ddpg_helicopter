...
def tau_sensitivity(tau, episodes=200):
...
def main():
    ...
    for tau in taus:
        for i in range(3):
            ...
            ep_rewards, avg_rewards = tau_sensitivity(tau, episodes=episodes)

            if i == 0:
                rewards_dict[tau] = avg_rewards
            else:
                rewards_dict[tau] = np.vstack((rewards_dict[tau], avg_rewards))
    
    fig, ax = plt.subplots(1, 1, figsize=(8, 6))
    for tau, result in rewards_dict.items():
        mean_rewards = result.mean(axis=0)
        std_rewards = result.std(axis=0)

        plt.plot(mean_rewards, label=rf'$\tau$ = {tau:.2E}')
