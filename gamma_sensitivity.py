...
def gamma_sensitivity(gamma, episodes=200):
...
def main():
    ...
    for gamma in gammas:
        for i in range(3):
            ...
            ep_rewards, avg_rewards = gamma_sensitivity(gamma, episodes=episodes)

            if i == 0:
                rewards_dict[gamma] = avg_rewards
            else:
                rewards_dict[gamma] = np.vstack((rewards_dict[gamma], avg_rewards))
    
    fig, ax = plt.subplots(1, 1, figsize=(8, 6))
    for g, result in rewards_dict.items():
        mean_rewards = result.mean(axis=0)
        std_rewards = result.std(axis=0)

        plt.plot(mean_rewards, label=rf'$\gamma$ = {g:.2f}')
    ...
    plt.savefig('gamma_sensitivity.pdf')
    plt.close()
