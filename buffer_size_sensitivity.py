...
def buffer_size_sensitivity(buffer_size, episodes=200):

    print(f'============ Buffer size = {buffer_size:.2e} ===================')
    ...
    buffer = Buffer(num_states, num_actions, int(buffer_size), 64)
...
def main():
    ...
    buffer_sizes = np.logspace(3, 7, 5)
    ...
    for size in buffer_sizes:
        for i in range(2):
            ...
            ep_rewards, avg_rewards = buffer_size_sensitivity(size, episodes=episodes)

            if i == 0:
                rewards_dict[size] = avg_rewards
            else:
                rewards_dict[size] = np.vstack((rewards_dict[size], avg_rewards))

    fig, ax = plt.subplots(1, 1, figsize=(8, 6))
    for size, result in rewards_dict.items():
        mean_rewards = result.mean(axis=0)
        std_rewards = result.std(axis=0)

        df = pd.DataFrame(data={'mean': mean_rewards, 'std':std_rewards})
        df.to_csv(f'results_buffer_size_{int(size)}.csv')
        plt.plot(mean_rewards, label=f'Buffer size = {size:.2E}')
    ...
    plt.savefig('buffer_size_sensitivity.pdf')
    # plt.show()
    plt.close()
